...
const DEFAULT_CLUB_NAMES = ["Wetherill Park", "Bankstown", "580G", "Woolooware"];
...
export default {
  async fetch(request, env) {
    ...
    try {
      const clubSiteIds = await resolveClubSiteIds(env);
      const payments = await listPayments(env);
      const rolling = buildRollingRows(payments, clubSiteIds);
...
async function listSites(env) {
  ...
  for (let page = 0; page < maxPages; page += 1) {
    const url = new URL(`${baseUrl(env)}/sites`);
    url.searchParams.set("status", "active");
    url.searchParams.set("limit", "100");
    if (startingAfter) url.searchParams.set("starting_after", startingAfter);

    const response = await fetch(url, { headers: authHeaders(env) });
    if (!response.ok) throw new Error(`Hapana sites request failed: ${response.status}`);

    const body = await response.json();
    const pageSites = body.data || [];
    sites.push(...pageSites);

    if (!body.hasMore && !body.has_more) break;
    startingAfter = pageSites[pageSites.length - 1]?.id;
    if (!startingAfter) break;
  }
...
async function resolveClubSiteIds(env) {
  const configured = JSON.parse(env.CLUB_SITE_IDS || "{}");
  ...
  try {
    ...
    console.warn("Could not list Hapana sites; payment metadata will be used for club matching.", error);
    return {};
  }

  const clubNames = JSON.parse(env.CLUB_NAMES || JSON.stringify(DEFAULT_CLUB_NAMES));
  const resolved = {};

  for (const club of clubNames) {
    const match = sites.find((site) => sameClub(site.name, club) || sameClub(site.shortName, club));
    if (match) resolved[club] = match.id;
  }
...
function authHeaders(env) {
  ...
  if (env.HAPANA_AUTH_MODE === "basic" && env.HAPANA_API_KEY) {
...
function findClub(payment, clubBySiteId) {
  const siteIds = [
  ...
  for (const siteId of siteIds) {
    if (clubBySiteId[siteId]) return clubBySiteId[siteId];
  }
  ...
  return DEFAULT_CLUB_NAMES.find((club) => sameClub(haystack, club)) || null;
}
